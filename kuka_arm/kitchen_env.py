...
for w in range(0, imgW, stepX):
    for h in range(0, imgH, stepY):
        ...
        # todo not getting far wall balls!?!?!?
...
for i in range(10000000):
    ...
    all_ball_x_y_locations_above_floor = [(x[0], x[1]) for x in all_ball_locations
                                          if x[2] > floor_height]
    ...
    def set_closest_occupancy_grid(x, y):
    ...
    for ball in all_ball_x_y_locations_above_floor:
        set_closest_occupancy_grid(ball[0], ball[1])
    ...
    ax2.set_xticklabels([round(x, 1) for x in x_range])  # todo fix
    ...
    # todo
    # todo if key is pressed calculate occupancy grid
