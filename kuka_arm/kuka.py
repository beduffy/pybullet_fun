...
class Kuka:
    ...
    def reset(self):
        ...
        objects = p.loadSDF(os.path.join(currentdir, "kuka_iiwa/kuka_with_gripper2.sdf"))
    ...
    def applyAction(self, motorCommands):
        ...
        if (self.useInverseKinematics):
            ...
            if (self.useNullSpace == 1):
                if (self.useOrientation == 1):
                    jointPoses = p.calculateInverseKinematics(self.kukaUid,
                                                              self.kukaEndEffectorIndex, pos, orn,
                                                              self.ll, self.ul, self.jr, self.rp)
                else:
                    jointPoses = p.calculateInverseKinematics(self.kukaUid,
                                                              self.kukaEndEffectorIndex, pos,
                                                              lowerLimits=self.ll,
                                                              upperLimits=self.ul,
                                                              jointRanges=self.jr,
                                                              restPoses=self.rp)
            ...
            if (self.useSimulation):
                for i in range(self.kukaEndEffectorIndex + 1):
                    ...
                    p.setJointMotorControl2(bodyUniqueId=self.kukaUid, jointIndex=i,
                                            ...
                                            targetPosition=jointPoses[i], targetVelocity=0,
            ...
            p.setJointMotorControl2(self.kukaUid, 10, p.POSITION_CONTROL, targetPosition=0,
                                    force=self.fingerTipForce)
            p.setJointMotorControl2(self.kukaUid, 13, p.POSITION_CONTROL, targetPosition=0,
                                    force=self.fingerTipForce)
